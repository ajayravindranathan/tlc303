...
Change the date_extracted column data type to date.
...
Next click on the Date data type on the Date_extracted column and click on Edit date format.
...
Update the date format as shown in the screen shot below:
...
Click on **Publish & Visualize** on the top right corner

f. Create a line graph for the 

Click on Visualize on the left hand pane


Cick on Line graph on the line graph visual type in the visual types section
...
Drag Date extracted on the x-axis and 5g_downlink_traffic_volume_huaw on the y-axis
...
Click on the chevron icon on the top right to pull down the field wells
...
Change the aggregate for date_extracted to hours
...
Change the aggregate for the value field well(5g_downlink_traffic_volume_huaw) to average
...
The resulting graph should look like the below
...
Click on Add and then Add Insight (Top left)
...
Choose Anomaly detection and then click on Select
...
Drag Date Extracted, change its aggregate to hours and then drag 5g_downlink_traffic_volume_huaw and change its aggregate to average, select cellname_nrcell on the categories field
...
Click on Get started on the Insights pane.
...
The bue dot shows the anomaly in the preview.

Next pick all 4 fields on the Top contributorsfield as shown in the following screen shot:
...
You can also select the different cells to see the anomaly detection preview and top contributors change for that particular cell.
...
Select all in the cellname_nrcell field once done and then click on Save at the top right. On the next screen click on Run now.
...
This will take about 3min to run. The following screen should appear after it finishes processing. Click on Explore Anmalies.
...
This will bring up the following screen with a zoomed in view to the anomalous data against the timeline, showing the top contributors on the left hand pane.
...
You can drop the field well from the top to explore the configuration options available to see different controls available.
